...
function gulpSpritesmith(params) {
  ...
  // TODO: Use `through` instead of `Readable` here
  ...
  var onEnd = function (cb) {
    ...
    async.parallel([
      ...
      var normalSpriteData = spritesmith.processImages(normalSprites, spritesmithParams);
      var retinaSpriteData;
      if (retinaSprites) {
        retinaSpriteData = retinaSpritesmith.processImages(retinaSprites, retinaSpritesmithParams);
      }
      ...
      // TODO: Verify that our plugins work as expected -- otherwise, document `vinyl-buffer`
      normalSpriteData.img.on('error', function forwardImgError (err) {
        that.emit('error', err);
      });
      var imgFile = new gutil.File({
        path: imgName,
        contents: normalSpriteData.img
      });
      that.push(imgFile);
      ...
      if (retinaSpriteData) {
        var retinaImgFile = new gutil.File({
          path: retinaImgName,
          contents: retinaSpriteData.img
        });
        retinaSpriteData.img.on('error', function forwardImgError (err) {
          that.emit('error', err);
        });
      ...
      // Aggregate our normal and retina info
      normalSpriteData.info.on('error', function forwardInfoError (err) {
        that.emit('error', err);
      });
      if (retinaSpriteData) {
        retinaSpriteData.info.on('error', function forwardInfoError (err) {
          that.emit('error', err);
        });
      }
      // async.parallel(function handle
