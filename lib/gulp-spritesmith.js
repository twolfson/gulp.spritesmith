...
function gulpSpritesmith(params) {
  ...
  var onEnd = function (cb) {
    ...
    if (retinaSrcFilter) {
      ...
      retinaImages = [];
      var retinaSrcFilterMinimatches = Array.isArray(retinaSrcFilter) ? retinaSrcFilter : [retinaSrcFilter];
      retinaSrcFilterMinimatches = retinaSrcFilterMinimatches.map(function getMinimatch (pattern) {
        return new Minimatch(unrelative(cwd, pattern));
      });
      images = images.filter(function filterSrcFile (filepath) {
        // If we have a retina file, filter it out
        var matched = retinaSrcFilterMinimatches.some(function matchMinimatches (minimatch) {
          return minimatch.match(filepath);
        });
        if (matched) {
          retinaImages.push(filepath);
          return false;
